...
class Mysql(object):
    ...
    def __init__(self,dbpool = None):
        ...
        self._conn = Mysql.__getConn(dbpool)
        self._cursor = self._conn.cursor()
 
    @staticmethod
    def __getConn(dbpool):
        """
        ...
        if dbpool is 'DB166':
            print("已连接166")
            __pool = PooledDB(creator=pymysql, mincached=1 , maxcached=20 ,
                              host=Config.DBHOST166 , port=Config.DBPORT , user=Config.DBUSER166 , passwd=Config.DBPWD166 ,
                              db=Config.DBNAME166,use_unicode=False,charset=Config.DBCHAR,cursorclass=DictCursor)
            return __pool.connection()
